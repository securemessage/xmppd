...
/// Server disco#info features — the single source of truth for the server's
/// disco#info feature set. iq_handler's disco#info result is generated by
/// iterating this list (see writeDiscoFeatures), so the two can never drift.
/// Sorted alphabetically per XEP-0115 §5.1 step 3.
pub const SERVER_FEATURES = [_][]const u8{
...
/// Write the disco#info `<feature>` elements for SERVER_FEATURES.
/// Used by iq_handler's disco#info result so the advertised feature set and
/// the XEP-0115 ver hash input are the same list by construction.
...
test "writeDiscoFeatures: emitted feature set == SERVER_FEATURES" {
    var buf: [4096]u8 = undefined;
    var fbs = std.io.fixedBufferStream(&buf);
    try writeDiscoFeatures(fbs.writer());
    const xml = fbs.getWritten();

    // Exactly one <feature> element per SERVER_FEATURES entry, in order.
    var pos: usize = 0;
    for (SERVER_FEATURES) |feature| {
        var expected_buf: [256]u8 = undefined;
        const expected = try std.fmt.bufPrint(&expected_buf, "<feature var='{s}'/>", .{feature});
        const found = std.mem.indexOfPos(u8, xml, pos, expected) orelse
            return error.TestExpectedFeatureMissing;
        try std.testing.expectEqual(pos, found);
        pos = found + expected.len;
    }
    // No extra features beyond SERVER_FEATURES.
    try std.testing.expectEqual(xml.len, pos);
}
