...
pub const AuthHandler = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, store: *UserStore) AuthHandler {
    ...
    pub fn handleMessage(self: *AuthHandler, msg: protocol.Message) ?protocol.Message {
        return switch (msg) {
            .auth_request => |req| self.handleAuthRequest(req),
            .sasl_response => |resp| self.handleSaslResponse(resp),
            else => null, // Ignore responses (they go the other direction)
        };
    }

    fn handleAuthRequest(self: *AuthHandler, req: protocol.AuthRequest) protocol.Message {
    ...
    fn handlePlainAuth(self: *AuthHandler, req: protocol.AuthRequest) protocol.Message {
        ...
        // Look up user
        const creds = self.store.lookup(username) orelse {
    ...
    fn handleScramInit(self: *AuthHandler, req: protocol.AuthRequest) protocol.Message {
    ...
    fn handleSaslResponse(self: *AuthHandler, resp: protocol.SaslResponse) protocol.Message {
    ...
    pub fn cleanupSession(self: *AuthHandler, conn_id: u32) void {
        if (self.findScramSlot(conn_id)) |slot| {
            self.scram_sessions[slot].deinit();
        }
    }

    fn findOrCreateScramSlot(self: *AuthHandler, conn_id: u32) ?usize {
    ...
    fn findScramSlot(self: *AuthHandler, conn_id: u32) ?usize {
        for (&self.scram_sessions, 0..) |*s, i| {
            if (s.active and s.conn_id == conn_id) return i;
        }
        return null;
...
test "AuthHandler: PLAIN auth success" {
    ...
    const path = "/tmp/xmppd-test-auth-handler.db";
    ...
    defer store.deinit();
    try store.load();
    try store.addUser("alice", "secret123");
...
test "AuthHandler: PLAIN auth wrong password" {
    const allocator = std.testing.allocator;
    const path = "/tmp/xmppd-test-auth-handler-bad.db";
...
test "AuthHandler: SCRAM-SHA-256 full exchange" {
    const allocator = std.testing.allocator;
    const path = "/tmp/xmppd-test-auth-scram.db";
...
test "AuthHandler: SCRAM unknown user" {
    const allocator = std.testing.allocator;
    const path = "/tmp/xmppd-test-auth-scram-nouser.db";
